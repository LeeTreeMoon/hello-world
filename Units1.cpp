...
bool show() 
{
	...
	{
		case 1:
			printf("%-20s%-20s%-20s%-20s%-20s\n%-20s%-20s%-20s%-20s\n\n","英尺（in）","英寸（ft）","码（yd）","海里（sm）","毫米（mm）","米（m）","厘米（cm）","纳米（nm）","千米（km）");
			return true;
			break;
		case 2:
			printf("%-20s%-20s%-20s%-20s%-20s\n\n","毫秒（ms）","秒（s）","分钟（min）","小时（h）","天（d）");
			return true;
			break;
		case 3:
			printf("%-20s%-20s%-20s%-20s%-20s\n%-20s%-20s%-20s%-20s\n\n","克（g）","吨（t）","英钱（dwt）","英担（cwt）","盎司（oz）","千克（kg）","毫克（mg）","磅（lb）","打兰（dr）");
			return true;
			break;
		case 4:
			...
			break;
		case 5:		
			printf("%-20s%-20s%-20s%-20s%-20s\n%-20s%-20s%-20s%-20s%-20s\n\n","公顷（ha）","平方米（m2）","平方厘米（cm2）","平方英里（sq_mi）","平方竿（sq_rd）","平方英寸（sq_ft）","平方英尺（sq_in）","平方码（sq_yd）","平方毫米（mm2）","平方千米（km2）");
			return true;
			break;
		case 6: 
			printf("%-20s%-20s%-20s%-20s\n%-20s%-20s%-20s%-20s\n%-20s%-20s\n\n",
...
int main()
{
	...
	while(cin>>num>>origin>>end)
	{
	    ...
	    int flag;
	    cout<<"是否继续进行单位转换？请输入0或1（是：1；否：0）"<<endl;
		cin>>flag;
		if(flag==0) 
		break;
